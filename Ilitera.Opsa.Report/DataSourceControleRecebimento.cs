...
{
	public class DataSourceControleRecebimento
	{
		...
		public DataSourceControleRecebimento(int Mes, int Ano)
		{
			this.Mes = Mes;
			this.Ano = Ano;
		}

		public RptControleRecebimentoMensal GetReport()
		{
			RptControleRecebimentoMensal report = new RptControleRecebimentoMensal();	
		...
		private DataSet GetDataSource()
		{
			...
			DateTime MesAnoRecebimento = new DateTime(Ano, Mes, 1).AddMonths(1);
			...
			DataSet dsTotal = new ReceberPagar().ExecuteDataset("SELECT SUM(ValorPago) AS TotalRecebido "
												...
												+ " AND (MONTH(DataPagamento) = "+MesAnoRecebimento.Month+")"
												+ " AND (YEAR(DataPagamento) = "+MesAnoRecebimento.Year+")");

			if(dsTotal.Tables.Count>0 
				&& dsTotal.Tables[0].Rows.Count>0 
				&& dsTotal.Tables[0].Rows[0][0]!=System.DBNull.Value)
				totalRecebido = Convert.ToSingle(dsTotal.Tables[0].Rows[0][0]);

			listFaturamentos  = new Faturamento().Find("(MONTH(Emissao) = "+Mes
			...
			foreach(Faturamento faturamento in listFaturamentos)
			{
				...
				newRow["Titulo"]		= "Controle de Recebimento "+Mes.ToString("00")+"/"+Ano;
