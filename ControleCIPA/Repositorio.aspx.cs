...
namespace Ilitera.Net.ControleCIPA
{
    public partial class Repositorio : System.Web.UI.Page
    {
        ...
        protected Ilitera.Opsa.Data.CipaRepositorio xRepositorio;
        //private string tipo;
        ...
        protected void InicializaWebPageObjects()
        {
            ...
            if (Request["IdCIPARepositorio"].ToString().Trim() != "0")
            {
                xRepositorio = new Ilitera.Opsa.Data.CipaRepositorio();
                xRepositorio.Find(Convert.ToInt32(Request["IdCIPARepositorio"]));
            }
            else
            {
                xRepositorio = new Ilitera.Opsa.Data.CipaRepositorio();
                xRepositorio.Inicialize();
                xRepositorio.IdCipa = Convert.ToInt32(Request["IdCIPA"]);
            }
        }
        ...
        {
            btnExcluir.Attributes.Add("onClick", "javascript:return(confirm('Deseja realmente excluir este registro de reunião?'))");
        }
        ...
        public void PopulaTelaExame()
        {
            ...
            if (xRepositorio.Evento == null)
                txtEvento.Text = "";
            else
                txtEvento.Text = xRepositorio.Evento.Trim();

            if (xRepositorio.TipoReuniao == "O")
                rblResultado.SelectedValue = "O";
            else
                rblResultado.SelectedValue = "E";
        ...
        protected void btnOK_Click(object sender, System.EventArgs e)
        {
            try
            {
                ...
                xRepositorio.Evento = txtEvento.Text.Trim();

                xRepositorio.TipoReuniao = rblResultado.SelectedValue.ToString().Trim();
                ...
                st.Append("window.alert('Dados de Reunião foram salvos com sucesso!');");
                ...
                Response.Redirect("CipaRepositorio.aspx?IdCipa=" + Request["IdCipa"].ToString().Trim() + "&IdEmpregado=" + Session["Empregado"].ToString() + "&IdEmpresa=" + Session["Empresa"].ToString() + "&IdUsuario=" + user.IdUsuario.ToString() + "&Tipo=3");
        ...
        protected void btnExcluir_Click(object sender, System.EventArgs e)
        {
            ...
            try
            {
                ...
                st.Append("window.opener.document.getElementById('txtAuxiliar').value='atualiza';");
                st.Append("window.opener.document.forms[0].submit();");
                st.Append("window.alert('Registro de reunião foi deletado com sucesso!');");
