...
function NextTxt(txtCheck, txtNext, numCarac)
{
	if (txtCheck.value.length == numCarac && VerifiqueTAB == true)
...
{
    if (window.opener.top.window.allwin != undefined) 
    {
...
function addItemPopPop(janela)
{
	window.opener.top.opener.top.window.allwin[window.opener.top.opener.top.window.allwin.length] = janela;
	window.opener.top.opener.top.window.allwinfunc[window.opener.top.opener.top.window.allwinfunc.length] = janela;
}

function ProcessaEnter(evento, button)
{
	if (isEnter(evento))
		eval("document.getElementById('" + button + "').focus();");
}
		
function isEnter(evento)
{
	if (!evento)
		evento = window.event; //IE
	else if (!evento.keyCode)
		evento.keyCode = evento.which; //NN4
