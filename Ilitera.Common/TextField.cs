...
namespace Ilitera.Common
{
    ...
    public class TextField
    {
        ...
        public TextField(string Name, TypeCode DataType, bool Quoted)
            : this(Name, DataType, 4095, Quoted)
        {

        }

        public TextField(string Name, TypeCode DataType)
            : this(Name, DataType, 4095, false)
        {

        }

        public TextField(string Name, TypeCode DataType, int Length)
        ...
        public object Value
        {
            ...
            set
            {
                try
                {
                    ...
                    else if (m_DataType == TypeCode.Double)
                    {
                        m_Value = ((Double)Convert.ChangeType(value, m_DataType)) / 100;
                    }
                    else if (m_DataType == TypeCode.DateTime && value.ToString() == "0000-00-00")
                    {
                        m_Value = Convert.DBNull;
                    }
                    else if (m_DataType == TypeCode.DateTime)
                    {
                        string s;

                        if (value.ToString().Trim().Length == 6)
                            s = value.ToString().Substring(0, 2) + "/" + value.ToString().Substring(2, 2) + "/" + value.ToString().Substring(4, 2);
                        else
                            s = value.ToString();
