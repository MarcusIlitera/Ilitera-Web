...
{
    public partial class ExameNaoOcupacional_EY : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (exame.Id == 0)
                {
                    ...
                    txtHora.Text = System.DateTime.Now.ToString("HH:mm");

                    Entities.Usuario user = (Entities.Usuario)Session["usuarioLogado"];
                    Pessoa rPessoa = new Pessoa();
                    rPessoa.Find(user.IdPessoa);

                    if ( rPessoa.Id != 0 )  txt_Responsavel.Text = rPessoa.NomeCompleto;
        ...
        private void Carga_Queixas()
        {

            Ilitera.Data.Clientes_Clinicas xQueixas = new Ilitera.Data.Clientes_Clinicas();
            DataSet zDs = xQueixas.Trazer_Queixas();

            chk_Diagnostico.Items.Clear();
            //lst_Diagnostico.Items.Clear();

            for (int zCont = 0; zCont < zDs.Tables[0].Rows.Count; zCont++)
            {
                ListItem zItem = new ListItem(zDs.Tables[0].Rows[zCont]["Queixa"].ToString().Trim(), zDs.Tables[0].Rows[zCont]["Abreviatura"].ToString().Trim());
                chk_Diagnostico.Items.Add(zItem);

                //chk_Diagnostico.Items.Add(zDs.Tables[0].Rows[zCont]["Queixa"].ToString().Trim());
                // lst_Diagnostico.Items.Add(zDs.Tables[0].Rows[zCont]["Abreviatura"].ToString().Trim());
            }
        ...
        private void Carga_Medicacao()
        {

            Ilitera.Data.Clientes_Clinicas xMed = new Ilitera.Data.Clientes_Clinicas();
            DataSet zDs = xMed.Trazer_Medicacao();

            chk_Planejamento.Items.Clear();
            //lst_Planejamento.Items.Clear();

            for (int zCont = 0; zCont < zDs.Tables[0].Rows.Count; zCont++)
            {
                ListItem zItem = new ListItem(zDs.Tables[0].Rows[zCont]["Medicacao"].ToString().Trim(), zDs.Tables[0].Rows[zCont]["Abreviatura"].ToString().Trim());
                chk_Planejamento.Items.Add(zItem);

                //chk_Planejamento.Items.Add(zDs.Tables[0].Rows[zCont]["Medicacao"].ToString().Trim());
                //lst_Planejamento.Items.Add(zDs.Tables[0].Rows[zCont]["Abreviatura"].ToString().Trim());
            }
        ...
        private void PopulaExame()
        {
            ...
            exame.Hora_Atendimento = txtHora.Text.Trim();

            exame.Prontuario = txt_Avaliacao.Text.Trim();

            if (txtGlicose.Text == "") txtGlicose.Text = "0";
            ...
            userInput = txtSaturacao.Text.Trim();
            isValidNumber = Int32.TryParse(userInput, out result);
            if (isValidNumber == false) txtSaturacao.Text = "0";

            userInput = wnePulso.Text.Trim();
            isValidNumber = Int32.TryParse(userInput, out result);
            if (isValidNumber == false) wnePulso.Text = "0";

            userInput = wteSistolica.Text.Trim();
            isValidNumber = Int32.TryParse(userInput, out result);
            if (isValidNumber == false) wteSistolica.Text = "0";

            userInput = wteDiastolica.Text.Trim();
            isValidNumber = Int32.TryParse(userInput, out result);
            if (isValidNumber == false) wteDiastolica.Text = "0";



            exame.Altura = System.Convert.ToInt32(wneAltura.Text.Trim());
            exame.Peso = System.Convert.ToInt32(wnePeso.Text.Trim());
            exame.PressaoArterial = "";

            exame.Diastolica = System.Convert.ToInt16(wteDiastolica.Text.Trim());
            exame.Sistolica = System.Convert.ToInt16(wteSistolica.Text.Trim());

            exame.Pulso = System.Convert.ToInt16(wnePulso.Text.Trim());
            exame.Temperatura = System.Convert.ToSingle(wneTemperatura.Text.Trim());

            exame.Glicose = System.Convert.ToInt32(txtGlicose.Text.Trim()).ToString();


            exame.SaturacaoOxigenio = System.Convert.ToInt32(txtSaturacao.Text.Trim());
            ...
            if (chk_PS.Checked == true)
                exame.Conduzido_PS = "S";
            else
                exame.Conduzido_PS = "N";


            exame.ProntoSocorro = txt_PS.Text.Trim();
        ...
        public void PopulaTelaExame()
        {
            ...
            txtHora.Text = exame.Hora_Atendimento.ToString();

            txtSaturacao.Text = exame.SaturacaoOxigenio.ToString();
            ...
            wteSistolica.Text = exame.Sistolica.ToString();
            wteDiastolica.Text = exame.Diastolica.ToString();
            ...
            if (exame.Conduzido_PS != null)
            {
                if (exame.Conduzido_PS== "S")
                {
                    chk_PS.Checked = true;
                }
                else
                {
                    chk_PS.Checked = false;
                }
            }

            if (exame.ProntoSocorro != null)
            {
                txt_PS.Text = exame.ProntoSocorro;
            }
        ...
        protected void btnOK_Click(object sender, System.EventArgs e)
        {
            try
            {
                ...
                if (wteSistolica.Text.Trim() != "" && wteSistolica.Text.Trim() != "0")
                {
                    if (System.Convert.ToInt16(wteSistolica.Text) <= 40)
                    {
                    ...
                    {
                        if (wteDiastolica.Text.Trim() == "" || wteDiastolica.Text.Trim() == "0")
                        {
                        ...
                        {
                            if (System.Convert.ToInt16(wteDiastolica.Text) <= 40)
                            {
                                MsgBox1.Show("Ilitera.Net", "Valor Diastólica Inválido", null,
                                new EO.Web.MsgBoxButton("OK"));
                                return;
                            ...
                            {
                                if (System.Convert.ToInt16(wteDiastolica.Text) >= System.Convert.ToInt16(wteSistolica.Text))
                                {
                                    MsgBox1.Show("Ilitera.Net", "Valor Diastólica Inválido", null,
                                    new EO.Web.MsgBoxButton("OK"));
                                    return;
                ...
                //{
                //    wneAltura.Text = "0";
                //    MsgBox1.Show("Ilitera.Net", "Valor altura Inválido", null,
                //    new EO.Web.MsgBoxButton("OK"));
                //    return;
                //}
                ...
                //{
                //    wnePeso.Text = "0";
                //    MsgBox1.Show("Ilitera.Net", "Valor Peso Inválido", null,
                //    new EO.Web.MsgBoxButton("OK"));
                //    return;
                //}
                ...
                //{
                //    txtSaturacao.Text = "0";
                //    MsgBox1.Show("Ilitera.Net", "Valor saturação Inválido", null,
                //    new EO.Web.MsgBoxButton("OK"));
                //    return;
                //}
                ...
                //{
                //    wnePulso.Text = "0";
                //    MsgBox1.Show("Ilitera.Net", "Valor pulso Inválido", null,
                //    new EO.Web.MsgBoxButton("OK"));
                //    return;
                //}
