...
namespace Ilitera.Sied.Report
{
    ...
    public class DataSourceInsalubridade : DataSourceBase
    {
        ...
        public DataSourceInsalubridade(Empregado empregado) 
            : this(empregado, (int)IndTipoPPRA.PPRA)
        {
            
        }

        public DataSourceInsalubridade(Empregado empregado,  int indTipoPPRA)
            : this(empregado, EmpregadoFuncao.GetEmpregadoFuncao(empregado), indTipoPPRA)
        {

        }

        public DataSourceInsalubridade(EmpregadoFuncao empregadoFuncao, int indTipoPPRA)
            : this(empregadoFuncao.nID_EMPREGADO, empregadoFuncao, indTipoPPRA)
        {

        }

        public DataSourceInsalubridade(EmpregadoFuncao empregadoFuncao, LaudoTecnico laudoTecnico, int indTipoPPRA)
            :
        ...
        public DataSourceInsalubridade(Empregado empregado, EmpregadoFuncao empregadoFuncao, int indTipoPPRA)
            :
        ...
        public DataSourceInsalubridade(Empregado empregado, LaudoTecnico laudoTecnico, int indTipoPPRA)
            :
            this(   empregado, 
        ...
        public DataSourceInsalubridade(ArrayList listEmpregados, int indTipoPPRA)
        {
            this.indTipoPPRA = indTipoPPRA;
        ...
        private void PopularDataSet(ArrayList listEmpregados)
        {
            ...
            foreach (Empregado empregado in listEmpregados)
            {
                ...
                LaudoTecnico laudoTecnico = empregadoFuncao.GetLaudoTecnico();
                                         ...
                                         DateTime dtAssinaturaLaudo)
        {
            ...
            if (ComFoto)
            {
                string pathFoto = Ilitera.Common.Fotos.PathFoto_Uri(empregado.FotoEmpregado());
            ...
            newRow["NivelRuido"] = ruido.tVL_MED + " dB(A)";
            ...
            if (IsPpraPrevidenciario)
            {
                if (empregadoFuncao.IsPertenceAoPeriodo85())
                    newRow["AcimaLimite85"] = (ruido.tVL_MED > 85) ? "SIM" : "NÃO";
                else
                    newRow["AcimaLimite85"] = "-";

                if (empregadoFuncao.IsPertenceAoPeriodo90())
                    newRow["AcimaLimite90"] = (ruido.tVL_MED > 90) ? "SIM" : "NÃO";
                else
                    newRow["AcimaLimite90"] = "-";

                if (empregadoFuncao.IsPertenceAoPeriodo80())
                    newRow["AcimaLimite80"] = (ruido.tVL_MED > 80) ? "SIM" : "NÃO";
                else
            ...
            if (IsPpraPrevidenciario)
                newRow["DescricaoEPC"] = ghe.Epc().Replace(Ghe.strInexistente, @"A utilização de EPI é admitida no âmbito do programa de gerenciamento dos riscos ambientais em situações de trabalho que existem inviabilidade técnica insuficiência ou interinidade para implementação de EPC, conforme autorizam as normas legais – art. 171, V, “a”, da IN/INSS PR n° 11/06 c/c art. 383, § 2º, IN/SRP n° 03/05.");
            else
                newRow["DescricaoEPC"] = ghe.Epc();
            ...
            //if (IsPpraPrevidenciario)
            //newRow["Conclusao"] = empregadoFuncao.GetConclusaoInsalubridade();
            //else
            //   newRow["Conclusao"] = ghe.Conclusao(cliente);

            if (ghe.tDS_CNCL_FUNC != string.Empty)
                newRow["Conclusao"] = ghe.tDS_CNCL_FUNC;
            else
            {
                newRow["Conclusao"] = ghe.Conclusao(cliente);
            }
            ...
            newRow["AssinaturaDataLaudo"] = dtAssinaturaLaudo;
        }
        #endregion
        ...
        private void PopularRowAgentesQuimicos(Empregado empregado, Ghe ghe)
        {
            ...
            foreach (PPRA agenteQuimico in alAgentesQuimicos)
            {
                ...
                newRowAgentesQuimicos["AvaliacaoQuantitativa"] = agenteQuimico.GetAvaliacaoQuantitativa();
