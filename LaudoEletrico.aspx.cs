...
namespace Ilitera.Net
{
	...
	/// 
	/// </summary>
    public partial class LaudoEletrico : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
            try
            {
                ...
                Ilitera.Opsa.Data.LaudoEletrico zLaudo = new Ilitera.Opsa.Data.LaudoEletrico();                
                zLaudo.Find(System.Convert.ToInt32(Request["IdLaudoEletrico"].ToString()));
                ...
                DataSet zDs = new LaudoEletrico_Adequacao_Item().Get(" IdLaudoEletrico_Adequacao in ( Select IdLaudoEletrico_Adequacao from LaudoEletrico_Adequacao where IdLaudoEletrico = " + Request["IdLaudoEletrico"].ToString() + " ) ");

                if (zDs.Tables[0].Rows.Count > 0) zTipo = "N";
                ...
                if (zTipo == "N")
                {
                    reports[i++] = new DataSourceLaudoEletrico2(zCliente, zLaudo.Id).GetReport_Intro();
                    reports[i++] = new DataSourceLaudoEletrico2(zCliente, zLaudo.Id).GetReport();
                }
                else
                {
                    reports[i++] = new DataSourceLaudoEletrico2(zCliente, zLaudo.Id).GetReport_Old();
                    reports[i++] = new DataSourceDocumentoBase(zCliente, zLaudo.Id).GetReport();
                }


                reports[i++] = new DataSourceLaudoEletrico2(zCliente, zLaudo.Id).GetReport_Conclusao();
                ...
                if (zLaudo.Anexo1 != null)
                {
                    ...
                    if (zLaudo.Anexo1.Trim() != "" || zLaudo.Anexo2.Trim() != "" || zLaudo.Anexo3.Trim() != "" || zLaudo.Anexo4.Trim() != "")
                    {
                        ...
                        reports[i++] = new Ilitera.Sied.Report.DataSourcePPRAAnexo(zLaudo.IdCliente).GetReport_LaudoEletrico(zLaudo.Id);
                    }
                }
